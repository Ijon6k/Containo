...
export const VolumeCard = ({ volume, updateVolume, removeVolume }: VolumeCardProps) => {
  return (
    <div className="border-b border-border last:border-b-0 pb-4 mb-4 last:mb-0 last:pb-0 hover:bg-surface2/20 transition-colors -mx-5 px-5 p-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <button
            onClick={() => updateVolume(volume.id, 'type', volume.type === 'named' ? 'bind' : 'named')}
          ...
          >
            {volume.type === 'named' ? 'Named volume' : 'Bind mount'}
          </button>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-3 mt-3">
        <div>
          <label className="text-[12px] font-medium text-text-tertiary block mb-1">
            {volume.type === 'named' ? 'Volume name' : 'Host path'}
          </label>
          <input
            placeholder={volume.type === 'named' ? 'my-data' : './data'}
            value={volume.source}
