import { NextResponse } from "next/server";
import { docker } from "@/lib/core/docker";
import { withErrorHandler } from "@/lib/utils/api-handler";
import { logger } from "@/lib/core/logger";

export const GET = withErrorHandler(async () => {
  const images = await docker.listImages();
  const formattedImages = images.map((img: any) => ({
    id: img.Id.split(":")[1].substring(0, 12),
    fullId: img.Id,
    repoTags: img.RepoTags || ["<none>:<none>"],
    size: img.Size,
    created: img.Created,
    containers: img.Containers === -1 ? 0 : img.Containers,
  }));

  return NextResponse.json(formattedImages);
});

export const DELETE = withErrorHandler(async (request: Request) => {
  const { searchParams } = new URL(request.url);
  const id = searchParams.get("id");

  if (!id) {
    return NextResponse.json(
      { error: "Image ID is required" },
      { status: 400 },
    );
  }

  const force = searchParams.get("force") === "true";
  const image = docker.getImage(id);

  // Resolve image name for logging
  let imageName = id;
  try {
    const info = await image.inspect();
    imageName = info.RepoTags?.[0] || id.substring(0, 12);
  } catch {}

  logger.info("API", `Deleting image '${imageName}' (force: ${force})`);
  await image.remove({ force });

  logger.success("API", `Image '${imageName}' deleted`);
  return NextResponse.json({ success: true });
});
