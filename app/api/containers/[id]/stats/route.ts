...
// Legacy REST endpoint — streams per-container stats via SSE.
// Used as an alternative to the WebSocket stats:subscribe flow.
    ...
    async start(controller) {
      ...
      try {
          ...
          for (const line of lines) {
            try {
              ...
              if (systemDelta > 0.0 && cpuDelta > 0.0) {
                cpuPercent =
                  (cpuDelta / systemDelta) * data.cpu_stats.online_cpus * 100.0;
              ...
              const memPercent = limitMemory
                ? (usedMemory / limitMemory) * 100.0
                : 0;
              ...
              const statsEvent = {
                id,
                cpuPercentage: isNaN(cpuPercent)
                  ? 0
                ...
                memoryPercentage: isNaN(memPercent)
                  ? 0
                  : Number(memPercent.toFixed(2)),
        ...
        statsStream.on("error", (err: any) => {
          logger.error("API", `Stats stream error for container ${id}`, err);
        ...
        logger.error(
          "API",
          `Failed to start stats stream for container ${id}`,
          err,
        );
        controller.error(err);
      }
