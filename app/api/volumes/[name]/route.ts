...
// Exports a volume as a .tar download via an ephemeral alpine helper.
// The helper mounts the volume read-only, archives it, and is destroyed after.
  ...
  async (
    ...
    const cleanup = async () => {
      try {
        ...
        logger.error("API", "Failed to cleanup backup helper", e);
      }
    };
