...
export function WIPWrapper({ children, label = 'Coming Soon' }: WIPWrapperProps) {
  return (
    <div className="relative group overflow-hidden rounded-sm">
      <div className="absolute inset-0 z-50 flex items-center justify-center bg-surface2/60 backdrop-blur-sm border border-white/5 cursor-not-allowed select-none group-hover:bg-surface2/70 transition-colors duration-300">
        <motion.div
        ...
        >
          ...
          <span className="text-[12px] text-brand">{label}</span>
        </motion.div>
      </div>
